...
float gMoveFront = 0.0f;
float gMoveBack = 0.0f;
float gMoveLeft = 0.0f;
float gMoveRight = 0.0f;
float gMoveUp = 0.0f;
float gMoveDown = 0.0f;
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    while (!done)
    {
        ...
        gMoveFront = rcClamp(gMoveFront + dt * 4 * ((keystate[SDL_SCANCODE_W] || keystate[SDL_SCANCODE_UP]) ? 1 : -1), 0.0f, 1.0f);
        gMoveLeft = rcClamp(gMoveLeft + dt * 4 * ((keystate[SDL_SCANCODE_A] || keystate[SDL_SCANCODE_LEFT]) ? 1 : -1), 0.0f, 1.0f);
        gMoveBack = rcClamp(gMoveBack + dt * 4 * ((keystate[SDL_SCANCODE_S] || keystate[SDL_SCANCODE_DOWN]) ? 1 : -1), 0.0f, 1.0f);
        gMoveRight = rcClamp(gMoveRight + dt * 4 * ((keystate[SDL_SCANCODE_D] || keystate[SDL_SCANCODE_RIGHT]) ? 1 : -1), 0.0f, 1.0f);
        gMoveUp = rcClamp(gMoveUp + dt * 4 * ((keystate[SDL_SCANCODE_Q] || keystate[SDL_SCANCODE_PAGEUP]) ? 1 : -1), 0.0f, 1.0f);
        gMoveDown = rcClamp(gMoveDown + dt * 4 * ((keystate[SDL_SCANCODE_E] || keystate[SDL_SCANCODE_PAGEDOWN]) ? 1 : -1), 0.0f, 1.0f);
        ...
        if (sample && sample->getTool() && sample->getTool()->type() != TOOL_MY_NAV_MESH_SCENE) {
            float movex = (gMoveRight - gMoveLeft) * keybSpeed * dt;
            float movey = (gMoveBack - gMoveFront) * keybSpeed * dt + scrollZoom * 2.0f;
            scrollZoom = 0;
            ...
            cameraPos[1] += (gMoveUp - gMoveDown) * keybSpeed * dt;
        }
